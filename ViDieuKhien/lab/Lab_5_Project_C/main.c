...
void SysTick_Init(uint32_t ticks) {
	RCC->ICSCR &=~(0x07<<13);//clear bit 13 14 15 cua thanh ghi ICSCR ve 0 thanh ghi 32 bit
	RCC->ICSCR |=0x03<<14; // set bit 14 15 cua thanh ghi ICSCR
	// Disable SysTick IRQ & SysTick counter
	SysTick->CTRL = 0;
	
	// Set reload register
	SysTick->LOAD = ticks/10 - 1;
...
//	
	GPIO_PortC_Output_Pin_Init(0); 
  GPIOC->ODR  |= 1<<3;	

  SysTick_Init(418);	// Init SystTick timer to generate interrupts at every 1 ms (uP clock at default MSI 2.097 MHz)  
	
	while(1) {
		Delay(30);  // delay 50*1ms
		//GPIO_PortB_Pin_Toggle(GREEN_LED);	// Toggle the green LED
		//GPIO_PortB_Pin_Toggle(BLUE_LED);	// Toggle the blue LED	

	  GPIO_PortC_Pin_Toggle(3);	// Toggle the green LED
		Delay(10);
		GPIO_PortC_Pin_Toggle(3);	//		
	}
}