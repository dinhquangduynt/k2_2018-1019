...
// Enable the clock to GPIO Ports C&D
...
/* Init a pin of Port C as digital output, output type of push-pull */
void GPIO_PortC_Output_Pin_Init(int pinID) {
	// Set pin as digital output (01)
	GPIOC->MODER &= ~(0x03<<(2*pinID));
	GPIOC->MODER |=   0x01<<(2*pinID);
	
	// Set output type of pin as push-pull (0)
	GPIOC->OTYPER &= ~(0x01<<pinID);
	
	// Set pin's output speed (01 = 2MHz, 10 = 10MHz)
	GPIOC->OSPEEDR &= ~(0x03<<(2*pinID)); // Speed mask
...
#define LED  13	// BLUE  LED: connected to PB6
...
int main(void){
	...
	GPIO_PortC_Output_Pin_Init(LED); 	// Init the pins attached to the LEDs as digital outputs
	//GPIO_PortB_Output_Pin_Init(BLUE_LED); 		
	
	GPIO_PortD_Input_Pin_Init(pinPD2);	// Init the pin PD2 attached to the user button as digital input
	...
	while(1) {		
		if (GPIOD->IDR & (1<<2)) {		// User button pressed
			//while (GPIOD->IDR & 0x01) {} // wait until button released
			GPIOC->ODR  |= 1<<LED;	// Toggle the green LED
